...
use crate::{comms::Messages, virtio::{device_driver::DeviceDriver, virtqueue::DescriptorCell}, faux_blk::{self, FILE_STATE_FLAG, STATE_SUCCESS, FILE_READ}, poller::PollableQueue};
...
unsafe fn write_file_to_cell<const S: usize, P: PollableQueue + Copy + Clone>(driver: &mut DeviceDriver<S, P>, cell: &mut DescriptorCell) {
...
unsafe fn read_message<const S: usize, P: PollableQueue + Copy + Clone>(comms: &Sender<Messages>, driver: &mut DeviceDriver<S, P>, cell: &mut DescriptorCell, idx: u16) {
...
pub unsafe fn create_device_thread<const S: usize, P: PollableQueue + Copy + Clone>(ui_comms: Sender<Messages>, mut driver: DeviceDriver<S, P>) {
